---
name: asana-cli
description: Asana CLI (asana-cli) command reference and safety workflow — use when reading Asana workspaces, projects, tasks, or comments; writing tasks or comments; resolving configuration/aliases; or triaging an asana-cli failure.
...
| Inspect bundled-skill installations                | `asana-cli skill list --json`                                                                                                                                                                                                                          |
| Install the bundled skill                          | `asana-cli skill install claude-code\|codex\|copilot\|cursor\|gemini\|pi\|universal [--local] [--force]`                                                                                                                                               |
| Update installed bundled skills                    | `asana-cli skill update <agent>\|all [--local]`                                                                                                                                                                                                        |
| Uninstall the bundled skill                        | `asana-cli skill uninstall <agent> [--local]`                                                                                                                                                                                                          |
...
Skill management is filesystem-only: it never reads Asana authentication or
uses the network. Global scope is the default; `--local` selects the current project.
Install refuses to replace an existing file unless `--force` is explicit,
`update all` skips absent targets, and CLI upgrades never update skills
implicitly.

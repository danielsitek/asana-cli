...
- Never hand-edit `homebrew-tap/Formula/asana-cli.rb`; use the tap's updater against the published release and its `SHA256SUMS`, per `docs/release-process.md`, to avoid transcription drift.
...
- When merging a pull request, delete its source branch remotely and locally after verifying the merge and updating `main`; merged feature branches must not remain stale.
- When asked to create repository tasks, use GitHub Issues unless the user explicitly asks for local task files; do not infer `docs/tasks/` from the word "task" alone.
